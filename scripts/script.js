 $(document).ready(function () {
    $.getJSON('http://demo3340210.mockable.io/api/products/?format=json', function (data) {
        ...
        $("#renderList").append(
                                '<li role="presentation"><a class="btn btn-default itemSelector" >'+data.products[i].title+'</a></li>'
                            );
        }
        ...
        $(".itemSelector").click(function () {
                ...
                $("#post").click(function(){
                    $("#alert").empty();
                    $("#alert").append('<div class="panel panel-danger"><div class="panel-body" >You must be logged in to post a comment!</div></div>');
                    $.ajax({
